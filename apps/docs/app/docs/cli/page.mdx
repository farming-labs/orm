...
The repo’s package release flow is also `bumpp`-driven now:
...
The current `release` script creates a version commit like `chore: release v0.0.x`
and a matching git tag. `release:latest` now runs that bump flow and then publishes
the new version to npm. `publish:*` is still useful when you need to retry a publish
