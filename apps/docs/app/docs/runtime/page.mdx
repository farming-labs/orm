...
That helper keeps `createOrm(...)` itself explicit while giving higher-level
integrations a clean “accept the raw client and normalize it” entrypoint.

The same package also exposes runtime-aware setup helpers:

- `pushSchema(...)`
- `applySchema(...)`
- `bootstrapDatabase(...)`

Those helpers use the same runtime detection layer to prepare direct SQL,
Prisma, MongoDB, Mongoose, Drizzle, and Kysely runtimes without inventing a
...
- `pushSchema(...)` from `@farming-labs/orm-runtime`
- `applySchema(...)` from `@farming-labs/orm-runtime`
- `bootstrapDatabase(...)` from `@farming-labs/orm-runtime`
