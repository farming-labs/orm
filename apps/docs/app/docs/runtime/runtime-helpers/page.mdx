...
The same package also owns the runtime-aware setup flow.

```ts title="bootstrap-database.ts"
import { bootstrapDatabase } from "@farming-labs/orm-runtime";
...
import { pushSchema } from "@farming-labs/orm-runtime";
...
Use `pushSchema(...)`, `applySchema(...)`, or `bootstrapDatabase(...)` when you
also want the runtime helper package to prepare the live database.
