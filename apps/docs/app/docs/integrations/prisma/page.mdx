...
still handled by the Farming ORM runtime layer itself. It does not rely on
Prisma implicit many-to-many shortcuts.
...
is currently exercised against SQLite.
...
The repo currently verifies Prisma support in two ways:

- fast unit tests against a recording Prisma-like delegate layer
- a real local integration test that generates a real `PrismaClient` and runs
  one-to-one, one-to-many, belongs-to, many-to-many, mutations, and transaction
  rollback against local SQLite
...
The Prisma runtime package now exists. The next runtime-driver layers are:

- Drizzle
- Kysely

The important constraint is that they should plug into the same query contract
instead of redefining the API.
