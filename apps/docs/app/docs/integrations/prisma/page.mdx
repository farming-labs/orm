...
that logical relation shape natively. The clearest example today is explicit
join-table `manyToMany(...)` convenience traversal, which still stays on the
...
belongs-to, many-to-many, mutations, and transaction rollback against local
