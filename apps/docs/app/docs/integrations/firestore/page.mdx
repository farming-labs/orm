...
The Firestore package ships with two test paths:

- fast in-memory contract tests that run by default
- optional real-backend tests for an emulator or a cloud project

The real suite is skipped unless one of the Firestore auth environments is
present.
