...
describe("@farming-labs/orm core", () => {
  ...
  it("supports numeric ids and schema-qualified tables in the manifest and generators", () => {
    ...
    expect(manifest.models.auditEvent.fields.id.generated).toBe("increment");
    ...
    expect(drizzle).toContain("import { pgTable, pgSchema, serial, text");
  ...
  it("rejects implicit schema-qualified table strings and invalid tableName parts", () => {
    const dottedSchema = defineSchema({
      user: model({
        table: "auth.users" as any,
        fields: {
          id: id(),
        },
      }),
    ...
    expect(() => createManifest(dottedSchema)).toThrow(
      'Schema-qualified table strings are not supported. Use tableName("users", { schema: "auth" }) instead.',
    );
    expect(() => tableName("users", { schema: "auth.internal" })).toThrow(
      'tableName() schema name values cannot contain ".". Pass the table name and schema separately.',
    );
  });
