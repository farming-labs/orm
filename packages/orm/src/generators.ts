...
function drizzleImports(dialect: DrizzleGenerationOptions["dialect"], manifest: SchemaManifest) {
  ...
  const needsSerial = models.some((model) =>
    Object.values(model.fields).some(
      (field) =>
        field.kind === "id" && field.idType === "integer" && field.generated === "increment",
    ),
  );
...
function drizzleColumn(
  ...
  if (field.kind === "id") {
    if (field.idType === "integer") {
      if (dialect === "pg" && field.generated === "increment") {
        return `serial("${field.column}").primaryKey()`;
      }
      if (dialect === "mysql") {
        return field.generated === "increment"
          ? `int("${field.column}").autoincrement().primaryKey()`
          : `int("${field.column}").primaryKey()`;
      }
      if (dialect === "sqlite" && field.generated === "increment") {
        return `integer("${field.column}").primaryKey({ autoIncrement: true })`;
      }
...
function sqlType(
  ...
  if (field.kind === "id") {
    if (field.idType === "integer") {
      if (field.generated === "increment" && dialect === "postgres") {
